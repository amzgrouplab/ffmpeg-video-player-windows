...
static int split_media_file(char *dst, char *src, __int64 start, __int64 end, PFN_SPC spc)
{
    ...
    if (start >= 0) {
        ...
        start    = av_rescale_q_rnd(start, tbms, tbvs, (enum AVRounding)(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
        end      = av_rescale_q_rnd(end  , tbms, tbvs, (enum AVRounding)(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
...
//          fprintf(stderr, "failed to read frame !\n");
        ...
        if (pkt.stream_index == streamidx) {
            ...
            if (spc) {
                current = av_rescale_q_rnd(pkt.pts - start, tbvs, tbms, (enum AVRounding)(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
        ...
        pkt.pts = av_rescale_q_rnd(pkt.pts, istream->time_base, ostream->time_base, (enum AVRounding)(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
        pkt.dts = av_rescale_q_rnd(pkt.dts, istream->time_base, ostream->time_base, (enum AVRounding)(AV_ROUND_NEAR_INF|AV_ROUND_PASS_MINMAX));
